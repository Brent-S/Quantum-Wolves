...
//		}
		...
		int[] RandOrd = MainActivity.getRandomOrdering(NumPlayers);
		for(int i = 0; i < NumPlayers; i++){
			...
			while(it.hasNext()){
				...
				UpdateProbabilities();
				CollapseAllDead();
				boolean GameOver = (CheckWin() != WinCodes.GameNotOver); // maybe this should check if there is only one state left, not
				if(GameOver) {
					i = NumPlayers; // breaks for loop
					break; // breaks while
				}
			}
		}
